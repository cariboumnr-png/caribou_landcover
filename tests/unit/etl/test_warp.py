...
'''Unit tests for spatial warping and grid snapping.'''
...
import landseg.etl.warp as warp_mod
...
def test_warp_sentinel2_to_lambert(dummy_geotiff_factory, tmp_path):
    ...
    Then: Output GeoTIFF has 10 bands in EPSG:3161 with exact canvas dimensions.
    '''
    ...
    canvas = canvas_mod.CanvasSpec(
        crs='EPSG:3161',
        resolution=20.0,
        bounds=(500000.0, 600000.0, 501000.0, 601000.0) # 50px x 50px
    )

    out_path = str(tmp_path / 'warped_s2.tif')
    warp_mod.warp_to_canvas(
...
def test_warp_categorical_labels_preserves_integer_values(dummy_geotiff_factory, tmp_path):
    ...
    Then: Output contains strictly integer class IDs (1-5, 255) without interpolation.
    '''
    ...
    out_path = str(tmp_path / 'warped_labels.tif')
    warp_mod.warp_to_canvas(
